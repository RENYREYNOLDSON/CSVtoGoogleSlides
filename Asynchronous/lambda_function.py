                            ...
                            for col in range(int(tables[i]["columns"])):
                                ...
                                for row in range(int(tables[i]["rows"])):
                                    if table_data[i][row][col]!="":
                                        empty=False
                                        #
                                        # DONT FORMAT VALUES WITH @ IN THEM!
                                        #
                                        #ADD $ signs and money format. REMOVE DECIMALS
                                        if '@' not in table_data[i][row][col] and "@" not in csv_data[counter][col]:
...
#Additional function to update the records 'Completed' field
def update_table(key,link):
    client = boto3.resource("dynamodb")
    table = client.Table("CSVtoSlides_Table")
    #In this table, find the relevant ID and update the completed tag
    table.update_item(Key={"key":key},UpdateExpression="set Completed=:s",ExpressionAttributeValues={":s":link})
    return
...
def lambda_handler(event, context):
    ...
    try:
        ...
        completed = data["Completed"]["S"]
        if completed!="False":
            print("EXITING AS ALREADY COMPLETED!")
            return

        ID = data["ID"]["S"]
        key = data["key"]["S"]

        token = data["accessToken"]["S"]
        CSVdata = data["CSVdata"]["S"]
        fileID = data["fileID"]["S"]
        outputName = data["outputName"]["S"]

        # Split the CSV data into rows using semicolon as delimiter
        rows = CSVdata.split('¦')
        ...
        link = CSVtoGSlideRipple(CSVfile,fileID,outputName,token).editSlides()
    except Exception:
        #Set 'Completed' in the DynamoDB record to the return value
        link="ERROR:"+str(traceback.format_exc())
    #Set 'Completed' in the DynamoDB record to the return value
    update_table(key,link)
...
>>>>>>> a62de97171b8c7a757ef2e9b58abdd6e61068434
